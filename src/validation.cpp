...
#if defined(NDEBUG)
# error "Bithao cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bithao Signed Message:\n";
using namespace std;
// Internal stuff
namespace {
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BHA additional fees, %d delta bytes\n",
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Bithao: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
