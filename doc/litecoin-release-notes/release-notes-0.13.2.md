Bithao Core version 0.13.2 is now available from:

  <https://download.bithao.org/bithao-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/cqcrytohub/bithao/issues>
...
updates will be released anymore. Without security updates, using a bithao
...
ECDSA signatures inside Bithao transactions now use validation using
...
Previous versions of Bithao Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bithao Core was to raise the default minimum
relay fee.

Bithao Core 0.13.2 will have a strict maximum size on the mempool. The
...
Bithao Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running bithaod without having to do any manual configuration.
...
Bithao Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bithao Core will relay transactions with
...
entirely in Bithao Core 0.13, and it is at this time undecided whether the
...
manual configuration. Bithao Core will also use Tor automatically to connect
...
Bithaod can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bithao
Core to use floating fees. By default, Bithao Core will use floating
fees.
...
(default: `0.0002` BHA/kB).

At all times, Bithao Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BHA.
Furthermore, Bithao Core will never create transactions paying less than
...
`bithao.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bithaorpc>
        ...
        # optional bypass bithaod rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< bithaorpc:<password>
    </Location>
...
Restarting bithaod will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in bithao.conf
...
For example:
    ...
    $ src/bithao-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Bithao Core code base has
...
- `bithao-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bithao-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Bithao scripting system that in combination with
...
Bithao transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bithao Core.
...
The two phases are distinct in the Bithao-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bithao wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bithao.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bithao block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bithaos they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bithaos being spent is exactly
...
- **Increased security for multisig:** Bithao addresses (both P2PKH addresses
  ...
  believed to be provided by Bithao's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bithao's current ledger (called the UTXO set),
  ...
  note that Bithao Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bithao users to individually opt-in to almost any change in the Bithao
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bithao Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bithao
client to Bithao Core 0.13.2 to signal for segwit support. When a
...
Since Bithao Core 0.10.0, nodes have defaulted to only relaying and
...
- `bithao-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bithao-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
