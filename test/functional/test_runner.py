...
def main():
    ...
    tmpdir = "%s/bithao_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and bithaod must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "bithaod"]) is not None:
            print("%sWARNING!%s There is already a bithaod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITHAOD" not in os.environ:
        os.environ["BITHAOD"] = build_dir + '/src/bithaod' + exeext
        os.environ["BITHAOCLI"] = build_dir + '/src/bithao-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `bithao-cli help` (`rpc_interface.txt`).
